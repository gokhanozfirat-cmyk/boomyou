-- Finalize one-time archiving by message id (participant-guarded).
-- Removes content-shape dependency so one-time media from older payloads is still consumable.
